---
title: "CIC Project: Histone ChIP-seq Analysis"
author: "Stephen Lee"
...
[QC Tutorial by James Topham](/projects/marralab_cic_prj/reports/cic_qc_tutorial.html)

## **Section 1: Using Make** <a name="section1"></a>

----

Most of the code for performing QC was written by James Topham - this includes all of section 1 and most of section 2. Much of the R code was re-written by me in my preferred style of coding, both for my personal practice and to see whether I could reproduce the results. James' original QC tutorial markdown can be found [here](/projects/marralab_cic_prj/reports/cic_qc_tutorial.html).
...
### **Coverage across Chromosome 1** <a name="chr1"></a>

One of the QC experiments involves visualizing coverage across chromosome 1, for each library and its respective input. This helps us see what the distribution of reads looks like across broad, continuous regions of the genome. IP libraries should look "spikey", while input libraries should have a relatively uniform distribution.

To do this experiment, we first need to bin chromosome 1 into non-overlapping bins of equal size (175 bp) using *bedtools windowMaker*. Then, given these bins, we will calculate the coverage of each library in each bin using *bedtools coverageBed*. Here's the code:
...
## **Section 2: QC Analysis in R** <a name="section2"></a>

----

### Set up directory
```{r}
setwd("/projects/marralab_cic_prj/histone_mod_chip/")
...
### **BAM file summary** <a name="stats"></a>
...
read_flagstats <- function(flagstats) {
	files <- read.delim(flagstats, sep = '\t', header = F, stringsAsFactors = F) %>% 
		...
		separate(lib, c("cell_line", "rep", "mark"), sep = "_")
...
saveRDS(flagstats, "R_objects/histone_mod_chip_flagstats.rds")

flagstats_m <- flagstats %>% 
	melt(c("cell_line", "rep", "mark")) %>% 
	tidyr::unite(sample, cell_line, rep, sep = "_")

flagstats_m$variable <- as.character(flagstats_m$variable)
flagstats_m$variable[flagstats_m$variable == "total"] <- "# QC-passed reads"
flagstats_m$variable[flagstats_m$variable == "mapped"] <- "# mapped reads"
flagstats_m$variable[flagstats_m$variable == "perc"] <- "% reads mapped"
flagstats_m$variable[flagstats_m$variable == "qc_fail"] <- "# QC-failed reads"
flagstats_m$variable[flagstats_m$variable == "dupe"] <- "# duplicate reads"
flagstats_m$variable[flagstats_m$variable == "dupe_perc"] <- "% duplicate reads"

flagstats_m$sample = as.factor(flagstats_m$sample)
flagstats_m$variable <- factor(flagstats_m$variable, levels = c("# QC-passed reads","# QC-failed reads", "# mapped reads", 
																																"% reads mapped", "# duplicate reads","% duplicate reads"))

pdf(file = "figures/qc/flagstats.pdf", height = 10, width = 15)
p <- ggplot(flagstats_m, aes(x = variable, y = value, color = mark, shape = sample)) + 
  geom_jitter(width = 0.1, alpha = 0.8, size = 4) +
  scale_shape_manual(values = 1:nlevels(flagstats_m$sample)) +
...
p
save_plot("figures/qc/flagstats.png", p, base_height = 10, base_width = 15)
dev.off()
```

<center><img src="/projects/marralab_cic_prj/histone_mod_chip/figures/qc/flagstats.png">
